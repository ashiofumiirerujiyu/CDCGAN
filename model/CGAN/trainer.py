...
class Trainer:
    ...
    def train(self, train_loader):
        ...
        for (x, y) in train_loader:
            ...
            real_loss = self.loss_func(self.model_d(x, y), real_labels)

            z = torch.randn(batch_size, self.noise_dim).to(self.device)
            fake_x = self.model_g(z, y)
            fake_loss = self.loss_func(self.model_d(fake_x, y), fake_labels)
            ...
            z = torch.randn(batch_size, self.noise_dim).to(self.device)
            fake_x = self.model_g(z, y)
            g_loss = self.loss_func(self.model_d(fake_x, y), real_labels)
